...
def apply_schema_v28(connection: sqlite3.Connection) -> None:
    ...
    connection.execute(
        ...
        CREATE TABLE IF NOT EXISTS rehearsal_sessions (
            ...
            tasks_total INTEGER NOT NULL DEFAULT 9 CHECK (tasks_total > 0),
            tasks_completed INTEGER NOT NULL DEFAULT 0 CHECK (tasks_completed BETWEEN 0 AND tasks_total),
            total_duration_seconds REAL NOT NULL CHECK (total_duration_seconds >= 0),
            avg_seq_score REAL NOT NULL CHECK (avg_seq_score BETWEEN 1 AND 7),
            trust_score REAL NOT NULL CHECK (trust_score BETWEEN 1 AND 5),
            est_minutes_saved INTEGER NOT NULL CHECK (est_minutes_saved >= 0),
    ...
    connection.execute(
        ...
        CREATE TABLE IF NOT EXISTS rehearsal_task_metrics (
            ...
            task_key TEXT NOT NULL CHECK (length(trim(task_key)) BETWEEN 1 AND 100),
            duration_seconds REAL NOT NULL CHECK (duration_seconds >= 0),
            seq_score INTEGER NOT NULL CHECK (seq_score BETWEEN 1 AND 7),
            hesitation_count INTEGER NOT NULL DEFAULT 0 CHECK (hesitation_count >= 0),
